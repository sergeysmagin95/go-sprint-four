...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	var speedInMsec = meanSpeed(action, duration) * kmhInMsec //средняя скорость в метрах в секунду
	return (((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedInMsec, 2.0) / height)) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
